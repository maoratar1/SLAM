...
and due to the tangent function behavior in the range  $\[0, \ {\pi \over 2\}]$. it holds that for error of  $\varepsilon$  and two angles  $\alpha > \beta$:  ${tg(\alpha + \varepsilon) \over tg(\alpha)} > {tg(\beta + \varepsilon) \over tg(\beta)}$. 
